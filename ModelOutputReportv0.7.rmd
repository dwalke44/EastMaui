...
nodes = read_excel(paste(inpath, "/No Diversions Final.xlsx", sep = ""))
nodes = nodes[ ,-c(2:7)]
nodes0 = read_excel(paste(inpath, "/Sugar Diversion Final.xlsx", sep = ""))
nodes0 = nodes0[ ,-c(2:7)]
nodes1 = read_excel(paste(inpath, "/IIFS2008 Final.xlsx", sep = ""))
nodes1 = nodes1[ ,-c(2:7)]
nodes2 = read_excel(paste(inpath, "/IIFS2008_plus_taro Final.xlsx", sep = ""))
nodes2 = nodes2[ ,-c(2:7)]
...
instreamOut = instreamOut[ ,c(5, 1:4)]
...
inwshed = bind_rows(inwshed1, inwshed2, inwshed3, inwshed4 )
inwshed = t(inwshed)
colnames(inwshed)<-c("natural", "sugar", "IIFS2008", "IIFS2008plus")
...
inditch = bind_rows(inditch1, inditch2, inditch3, inditch4)
inditch = t(inditch)
colnames(inditch)<-c("natural", "sugar", "IIFS2008", "IIFS2008plus")
...
inditch.mgd = inditch.mgd[ , c(5, 1:4)]
...
check = bind_rows(wshedcheck1, wshedcheck2, wshedcheck3, wshedcheck4, )
check = t(check)
colnames(check)<-c("natural", "sugar", "IIFS2008", "IIFS2008plus")
...
springs = bind_rows(springs1, springs2, springs3, springs4)
springs = t(springs)
colnames(springs)<-c("natural", "sugar", "IIFS2008", "IIFS2008plus")
...
sinks = bind_rows(sinks1, sinks2, sinks3, sinks4)
sinks = t(sinks)
colnames(sinks)<- c("natural", "sugar", "IIFS2008", "IIFS2008plus")
...
output_list = list("watershedsInput" = waterOutput,
                   ...
                   "springs" = springs,
                   "sinks" = sinks)
file = paste(outpath, "water_outputs.0.7.xlsx", sep = "")
...
# # match WshedID with Insect Basin habitat data
...
leases = leases %>% summarise_at(vars(c(6:9)), sum)
taro = left_join(dsNodes, leasesByID, by = "BASINID") %>%
  left_join(inditch.mgd, by = "NodeID") %>%
  filter(NodeID<45 | NodeID>54) %>%
  group_by(Taro_ID)
taro = taro[complete.cases(taro),]
taro = taro %>% summarise_at(vars(c(6:9)), sum)
wshed.hab.total = data.frame(AllSpHab.nat[ ,9],
                             AllSpHab.sugar[ ,9],
                             AllSpHab.IIFS[ ,9],
                             AllSpHab.IIFSplus[ ,9])

Wshed_names = unique(wshedNames$Wshed_Name)
Wshed_names = na.omit(Wshed_names)
wshed.hab.total$WatershedName = Wshed_names
wshed.hab.total$WshedID = c(1:46)

colnames(wshed.hab.total)[1:4]<- c("natural", "sugar", "IIFS",
                                   "IIFSplus")
wshed.hab.total = wshed.hab.total[ ,c(6,5, 1:4)]
...
#   group_by(Lease_ID)
# leases = leases[complete.cases(leases), ]
# leases = leases %>% summarise_at(vars(c(5:8)), sum)
# 
# taro = left_join(dsNodes, leasesByID, by = "BASINID") %>%
#   left_join(inditch.mgd, by = "NodeID") %>%
#   filter(NodeID<45 | NodeID>54) %>%
